# State of the Art: Multi-Agent Personality Simulation with Large Language Models

#intro on the doc 

---

## 2. LLM-Based Dialogue Agents: Architecture and Known Failure Modes
...
## 3. In-Context Persona Prompting: Mechanisms and Systematic Effects

The foundational empirical question for this project is whether LLMs, when prompted to adopt a persona, actually change their behavior in measurable and systematic ways — or whether the surface prompt is cosmetic. Salewski et al. [2023] provide the most controlled investigation of this question.

Using **in-context impersonation** prompts of the form *"If you were a {persona}"* — with no fine-tuning — applied to Vicuna-13B and ChatGPT (GPT-3.5-turbo), Salewski et al. [2023] test three task paradigms: a multi-armed bandit exploration/exploitation game, the MMLU reasoning benchmark, and a vision-language fine-grained classification task via CLIP-embedded descriptions. The results are unambiguous across all three settings:

- **Developmental stage impersonation:** Agents impersonating older personas (up to age 20) achieve higher rewards in the bandit task (β = 0.17, p < 0.001 for Vicuna-13B; β = 0.26, p < 0.001 for Llama 2 replication), explore less, and exploit more — closely paralleling known human developmental trajectories. The effect plateaus for ages 20–60, again consistent with human adult cognitive patterns.
- **Expertise impersonation:** On MMLU, the ordering task expert > domain expert > non-domain expert > neutral holds consistently across all four domain categories and both models. Expert performance is approximately twice that of non-domain expert when using ChatGPT.
- **Linguistic register:** Text complexity of agent-generated descriptions rises from grade levels 3–4 (age 2 persona) to grade levels 9–10 (age 20 persona), providing a measurable linguistic fidelity dimension.

However, Salewski et al. [2023] also document a critical **bias problem**: persona prompts that specify demographic characteristics (race, gender) induce stereotyped behavioral patterns. White personas outperform Black personas on bird classification; woman personas outperform man personas on birds; man personas outperform woman personas on cars — in ways that reflect, rather than transcend, social stereotypes embedded in training data. ChatGPT shows larger bias effects than Vicuna-13B, attributed to broader fine-tuning data. This finding is a direct methodological warning for this project: if real-person persona prompts include demographic information, agents may respond to perturbations through the lens of demographic stereotypes rather than individual character consistency.
...
## 4. Personality Expression: Psychometric Validation of LLM Persona Agents

The central evaluation challenge for this project is measuring whether an agent has truly internalized a persona, as opposed to producing plausible but unfounded output. Jiang et al. [2024] (PersonaLLM) address this challenge most rigorously by applying the **Big Five Inventory (BFI)** — a validated psychometric instrument — to persona-conditioned LLMs.
...
**On BFI self-assessment (RQ1),** GPT-4 achieves large, statistically significant effect sizes across all five traits: Extraversion d = 5.47, Agreeableness d = 4.22, Conscientiousness d = 4.39, Neuroticism d = 5.17, Openness d = 6.30 (all p < 0.001). GPT-3.5-turbo shows similarly significant but less uniform effects. This confirms that persona-prompted LLMs do reliably encode assigned trait profiles in their self-reported attitudes.
...
**On human perception accuracy (RQ4),** human raters asked to predict the assigned Big Five traits from persona-generated stories achieve majority-vote accuracies of: Extraversion 0.84, Agreeableness 0.69, Conscientiousness 0.59, Neuroticism 0.59, Openness 0.53 — against a 0.5 binary chance baseline. Extraversion is by far the most robustly perceptible trait. Crucially, **informing raters that the author is an AI significantly reduces perceived personalness** (from 4.32 to 3.99 on a 5-point scale) and degrades personality prediction accuracy across all traits (Extraversion individual accuracy: 0.68 uninformed → 0.60 informed). This finding directly motivates the **blind evaluation protocol** in this project's experimental design: raters must not know whether they are reading AI-generated or human text.
...
## 5. Behavioral Fidelity: Evaluating Real-Person Agent Authenticity

Moving from personality traits to behavioral authenticity — whether an agent responds to real-world situations as the target individual would — Zhou et al. [2025] provide the closest existing work to the behavioral fidelity evaluation problem this project faces.

Their methodology constructs agents for 35 real public figures (politicians) using profile data from Wikipedia and Twitter/X, and evaluates agent responses against the figures' actual tweets in response to real-world news events, using the **RWERD (Real-World Events Responses Dataset)**. Event attitudes are labeled Positive, Neutral, or Negative by eight annotators (majority vote), providing a ground-truth signal for comparison.
...
**Personal detail features are the most critical driver of fidelity:** removing them causes the largest accuracy drop and increases variance. This has direct implications for this project's persona extraction design — the granularity and type of information extracted from source text corpora (beyond surface style) is the single most influential factor for downstream behavioral fidelity.

A secondary finding is that **GPT-4o produces systematically more neutral outputs** (20.0% neutral with full profile) compared to Llama-3-70B (2.6%), attributed to GPT-4o's Mixture-of-Experts architecture tending toward cautious, balanced responses. After a prompt intervention instructing the model not to choose neutral unless certain, GPT-4o NPPB accuracy jumps from 69.0% to **83.4%**, bringing it in line with Llama-3-70B. This model-specific conservatism is a key methodological caveat: when measuring perturbation responses, the LLM's own bias toward neutrality may systematically suppress directional character-consistent reactions, independent of persona quality.

The limitation of Zhou et al. [2025] is that behavioral fidelity is evaluated on a **single-dimension outcome** (positive/negative/neutral stance) and does not capture stylistic fidelity, multi-turn consistency, or group-level interaction dynamics.

---

## 6. Multi-Agent Social Simulation and Group Dynamics
...
- **Instruction-tuning side effects:** agents exhibit overly formal dialogue and excessive agreeableness, rarely declining requests — a systematic distortion of individual personality differences.
...
## 7. Evaluation Methodologies: A Cross-Paper Synthesis
...
- Zhou et al.'s [2025] event-driven evaluation, which tests stance on real-world events — though restricted to a single-dimension output
- Park et al.'s [2023] reactive plan updates and "reactions" interview question category — though targeting fictional agents
- Salewski et al.'s [2023] bandit task as an implicit perturbation-response scenario (changing reward probabilities)

This constitutes the clearest **gap** in the existing literature: no prior work evaluates whether persona-conditioned LLM agents respond to counterfactual event injections in a direction predictable from the seed person's known positions — which is precisely this project's H3 hypothesis.
